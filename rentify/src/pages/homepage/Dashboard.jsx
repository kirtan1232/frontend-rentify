...
import { getAllProducts } from "../../api/Api"; // Adjust the import path for the API
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {products.length > 0 ? (
                        products.map((product) => (
                            ...
                            >
                                <img
                                    src={`http://localhost:5000/rooms/${product.roomImage}`} // Ensure this is the correct path
                                    alt={product.roomDescription}
                                    className="w-full h-40 object-cover"
                                />

                                <div className="p-4">
                                    <h3 className="text-lg font-bold text-gray-800 mb-2">{product.roomDescription}</h3>
                                    <p className="text-gray-600">Rs. {product.rentPrice} · {product.status}</p>
                                </div>
                            </div>
                        ))
                    ) : (
